"""Event-listener interface for Karry.
...
:class:`KarryListener`. Any UI — Tkinter window, tray tooltip, or a
future web dashboard — can implement the protocol and subscribe.

Listeners are *always* invoked from Karry's worker threads (audio and
...
@runtime_checkable
class KarryListener(Protocol):
    """Observer interface for the Karry orchestrator.
