"""Tkinter transcript window for Karry.

Shows a live conversation view: the text Karry heard, the tools it
called, and the responses it spoke. Runs the ``Karry`` orchestrator
in a background thread; Tk's mainloop owns the main thread.

Design notes
------------
* All Tk widget updates go through :meth:`_dispatch` on the main
  thread. Karry publishes events from worker threads, so we route them
...
from karry_assistant.ui.events import KarryListener
from karry_assistant.utils.logging import configure_logging


class _QueueListener(KarryListener):
    """:class:`KarryListener` that posts every event onto a queue.

    Karry publishes events from worker threads; the window drains
...
class TranscriptWindow:
    """Main Tk window showing what Karry hears and how it responds."""
    ...
    def __init__(self) -> None:
        ...
        self._karry = Karry(self._settings, listener=listener)
        self._worker: threading.Thread | None = None

        self._root = tk.Tk()
        self._root.title("Karry")
    ...
    def _build_ui(self) -> None:
        ...
        self._hint = tk.Label(
            footer,
            text=f"Say '{self._settings.wake_phrases[0]}' to talk to Karry.",
            fg="#6B7280",
    ...
    def _render(self, evt: tuple) -> None:
        ...
        if kind == "status":
            ...
            self._append("assistant", "Karry", evt[1])
        elif kind == "error":
    ...
    def _start_worker(self) -> None:
        def _target() -> None:
            try:
                ...
                self._event_q.put(("error", f"Karry crashed: {exc}"))
